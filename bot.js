...
function calculateSellMegahAmount(reserveMEGAH, reserveUSDT, targetPrice) {
...
function calculateBuyMegahAmount(reserveMEGAH, reserveUSDT, targetPrice) {
    const R = parseFloat(ethers.utils.formatEther(reserveMEGAH));
    const U = parseFloat(ethers.utils.formatEther(reserveUSDT));
    const currentPrice = R / U;
    if (currentPrice <= targetPrice) return ethers.BigNumber.from(0);
...
async function executeSwap(routerAddress, dexName, tokenIn, tokenOut, amountIn, arah, hargaSaatIni, gasPrice) {
    ...
    const inSymbol = tokenIn === CONFIG.USDT ? 'USDT' : 'MEGAH';
    const outSymbol = tokenOut === CONFIG.USDT ? 'USDT' : 'MEGAH';
    const jumlahIn = ethers.utils.formatEther(amountIn);
    log(`${dexName} | ${arah} | Harga: ${hargaSaatIni.toFixed(2)} MEGAH/USDT | Jumlah: ${jumlahIn} ${inSymbol} → min ${ethers.utils.formatEther(amountOutMin)} ${outSymbol}`);
...
async function getAllDexData() {
    ...
    for (const dex of dexNames) {
        calls.push({
            ...
            callData: factoryIface.encodeFunctionData('getPair', [CONFIG.MEGAH, CONFIG.USDT])
        });
    }
    ...
    for (let i = 0; i < dexNames.length; i++) {
        ...
        let reserveMEGAH, reserveUSDT;
        if (token0 === CONFIG.MEGAH && token1 === CONFIG.USDT) {
            reserveMEGAH = reserves.reserve0;
            reserveUSDT = reserves.reserve1;
        } else if (token0 === CONFIG.USDT && token1 === CONFIG.MEGAH) {
...
async function processDexWithData(dexData, gasPrice) {
    ...
    const { dex, router, reserveMEGAH, reserveUSDT } = dexData;

    const price = reserveMEGAH.mul(ethers.constants.WeiPerEther).div(reserveUSDT);
    const priceNum = parseFloat(ethers.utils.formatEther(price));
    log(`📊 Harga di ${dex}: 1 USDT = ${priceNum.toFixed(2)} MEGAH`);

    if (priceNum < CONFIG.LOWER_BOUND) {
        log(`⬇️ ${dex} | Harga ${priceNum.toFixed(2)} < ${CONFIG.LOWER_BOUND} → akan MENJUAL MEGAH (target ${CONFIG.TARGET_SELL})`);
        const amountInMEGAH = calculateSellMegahAmount(reserveMEGAH, reserveUSDT, CONFIG.TARGET_SELL);
        if (amountInMEGAH.isZero()) {
            log(`⚠️ ${dex} | Jumlah MEGAH yang diperlukan nol, tidak ada aksi.`, 'WARN');
            return;
        }

        const tokenMEGAH = new ethers.Contract(CONFIG.MEGAH, ERC20_ABI, signer);
        const balanceMEGAH = await callWithRetry(() => tokenMEGAH.balanceOf(account), `balanceOf MEGAH`);
        if (balanceMEGAH.lt(amountInMEGAH)) {
            log(`⚠️ ${dex} | Saldo MEGAH tidak cukup (butuh ${ethers.utils.formatEther(amountInMEGAH)}, tersedia ${ethers.utils.formatEther(balanceMEGAH)}). Swap sebanyak saldo.`, 'WARN');
            if (balanceMEGAH.isZero()) {
                log(`❌ ${dex} | Saldo MEGAH kosong, tidak bisa menjual.`, 'WARN');
                return;
            }
            await executeSwap(router, dex, CONFIG.MEGAH, CONFIG.USDT, balanceMEGAH, 'JUAL MEGAH', priceNum, gasPrice);
        } else {
            await executeSwap(router, dex, CONFIG.MEGAH, CONFIG.USDT, amountInMEGAH, 'JUAL MEGAH', priceNum, gasPrice);
        }
    }
    else if (priceNum > CONFIG.UPPER_BOUND) {
        log(`⬆️ ${dex} | Harga ${priceNum.toFixed(2)} > ${CONFIG.UPPER_BOUND} → akan MEMBELI MEGAH (target ${CONFIG.TARGET_BUY})`);
        const amountInUSDT = calculateBuyMegahAmount(reserveMEGAH, reserveUSDT, CONFIG.TARGET_BUY);
        ...
        if (balanceUSDT.lt(amountInUSDT)) {
            ...
            await executeSwap(router, dex, CONFIG.USDT, CONFIG.MEGAH, balanceUSDT, 'BELI MEGAH', priceNum, gasPrice);
        } else {
            await executeSwap(router, dex, CONFIG.USDT, CONFIG.MEGAH, amountInUSDT, 'BELI MEGAH', priceNum, gasPrice);
        }
    }
    else {
    ...
    try {
        ...
        for (const dexData of dexDataList) {
            if (dexData) {
                ...
                log(`⚠️ Salah satu DEX tidak memiliki pair MEGAH/USDT`, 'WARN');
            }
        }
    } catch (e) {
